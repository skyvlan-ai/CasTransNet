        ...
        if isinstance(self.labels, dict):
            ...
            for label, name in self.labels.items():
                ...
                for metric in eval_metrics:
                    self.result[k][metric] = _funcs[metric](test=self.test, reference=self.reference,
                                                            confusion_matrix=self.confusion_matrix,
                                                            nan_for_nonexisting=self.nan_for_nonexisting, **metric_kwargs)
...
class NiftiEvaluator(Evaluator):
    ...
    def evaluate(self, test=None, reference=None, voxel_spacing=None, **metric_kwargs):
        ...
        return super(NiftiEvaluator, self).evaluate(**metric_kwargs)
