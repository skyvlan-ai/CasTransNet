...
# from nnunet.network_architecture.CASTransv1 import CASTransv1
from nnunet.network_architecture.CASTrans import CASTrans
from nnunet.network_architecture.CASTransv1 import CASTransv1
from nnunet.network_architecture.CASTransv1 import Mlp
...
class nnUNetTrainerV2_CASTransv1_acdc(nnUNetTrainer):
    """
    ...
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        assert self.model_size in ['Tiny', 'Base',
                                   'Large'], "error key words, or you can dismiss it and set it by yourself"
        if self.model_size == 'Tiny':
            self.embedding_dim = 96
            self.num_heads = [3, 6, 12, 24]
            if self.pretrain:
                self.pre_trained_weight = torch.load("/home/CPCANet/weight/b8best9185.model",
                                                     map_location='cpu')
        ...
        if self.model_size == 'Base':
            ...
            if self.pretrain:
                self.pre_trained_weight = torch.load("/home/CPCANet/DATASET/nnUNet_trained_models/nnUNet/2d/Task001_ACDC/nnUNetTrainerV2_CASTransv1_acdc__nnUNetPlansv2.1/fold_0/2262_MA_9219.model",
                                                     map_location='cpu')
        ...
        if self.model_size == 'Large':
            ...
            self.num_heads = [4, 8, 16, 32]
            if self.pretrain:
                self.pre_trained_weight = torch.load("/home/CPCANet/weight/",
                                                     map_location='cpu')
        ...
        if not self.was_initialized:
            ...
            if training:
                ...
                self.tr_gen, self.val_gen = get_moreDA_augmentation(
                    ...
                    seeds_val=range(int(self.data_aug_params['num_threads']) * 2)[
    ...
    def initialize_network(self):
        # Don't touch conv_op
        self.network = CASTransv1(self.config,
        ...
        if self.pretrain:
            ...
            for i in self.network.state_dict():
                if i.replace('decoder',
                             'encoder') in checkpoint:  # there is a mistake in the name of the key of the pretrained model
    ...
    def initialize_optimizer_and_scheduler(self):
        ...
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=self.initial_lr) #Adam
        tmp = 1290/(self.batch_size) * self.max_num_epochs
        self.scheduler = CosineAnnealingLR(self.optimizer, T_max=tmp, eta_min=1e-6)
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        a, b, c, d = 0.04, 0.1, 0.29, 0.57
        # # a, b, c, d = 0.1, 0.15, 0.25, 0.50
        # w, x, y, z = 1.0, 1.0, 1.0, 1.0
        if self.fp16:
            with autocast():
                # output = self.network(data)
                p1, p2, p3, p4 = self.network(data)
                # output = a*p1 + b*p2 + c*p3 + d*p4
                output = p4
                # output = w*p1 + x*p2 + y*p3 + z*p4
                del data
                # l = self.loss(output, target)
                l1 = self.loss(p1, target)
                l2 = self.loss(p2, target)
                l3 = self.loss(p3, target)
                l4 = self.loss(p4, target)
    ...
    def maybe_update_lr(self, epoch=None):
        ...
        # self.scheduler.step(self.val_eval_criterion_MA)
