...
from nnunet.evaluation.metrics import hausdorff_distance_95
...
from nnunet.training.dataloading.dataset_loading import load_dataset, DataLoader3D, DataLoader2D_unet2022, \
    unpack_dataset
...
class nnUNetTrainer(NetworkTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.loss = DC_and_CE_loss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {}, weight_ce=0.8,
                                   weight_dice=1.2)
    ...
    def get_basic_generators(self):
        ...
        if self.threeD:
            ...
            dl_tr = DataLoader2D_unet2022(self.dataset_tr, int(self.data_aug_params['num_threads']),
                                          batch_size=self.batch_size, patch_size=self.basic_generator_patch_size,
                                          final_patch_size=self.patch_size)
            dl_val = DataLoader2D_unet2022(self.dataset_val, int(self.data_aug_params['num_threads']) // 2,
    ...
    def run_online_evaluation(self, output, target):
        with torch.no_grad():
            ...
            output_seg = output_softmax.argmax(1)  # batch * 224*224
            target = target[:, 0]  #batch * 224*224
            ...
            hd95 = hausdorff_distance_95(test=output_seg, reference=target, voxel_spacing=None,
                                             connectivity=1)
            ...
            self.online_eval_hd95.append(hd95)
    ...
    def finish_online_evaluation(self):
        ...
        global_dc_per_class = [i for i in [2 * i / (2 * i + j + k) for i, j, k in
                               ...
                               if not np.isnan(i)]
        hd95_scores = []
        for sublist in self.online_eval_hd95:
            if not np.isnan(sublist):
                hd95_scores.append(sublist)
        average_hd95 = np.mean(hd95_scores)
        dice_avg = np.mean(global_dc_per_class)
        self.all_val_eval_metrics.append(dice_avg)
        ...
        self.print_to_log_file("Average HD95 scores:", str(average_hd95))
