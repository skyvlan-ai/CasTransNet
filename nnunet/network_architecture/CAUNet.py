...
class MSABlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 norm_layer=nn.LayerNorm, channelAttention_reduce=4):
        super().__init__()
        self.dim = dim
        ...
        # self.dwconv = SpatialBlock(in_channels=dim, out_channels=dim,channelAttention_reduce=channelAttention_reduce)
        self.ca = RepBlock(in_channels=dim, out_channels=dim, channelAttention_reduce=channelAttention_reduce)
    ...
    def forward(self, x, mask_matrix):
        ...
        # caf = F.pad(shortcut, (0, 0, 0, pad_r, 0, pad_b))
        _, Hp, Wp, _ = x.shape
        ...
        caf = caf.permute(0, 3, 1, 2).contiguous()
        caf = self.bn(caf)
        caf = self.ca(caf)

        # cha = cha.permute(0, 2, 3, 1).contiguous()
        # dw = window_partition(self.window_size)  # nW*B, window_size, window_size, C
        ...
        if self.shift_size > 0:
            saf = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
        else:
            saf = shifted_x

        if pad_r > 0 or pad_b > 0:
            saf = saf[:, :H, :W, :].contiguous()
        saf = saf.permute(0, 3, 1, 2).contiguous()
        fusion = self.pfb(saf, caf)
        fusion = fusion.permute(0, 2, 3, 1).contiguous()
        # x = x.view(B,  H * W, C)
...
class BasicLayer(nn.Module):
    def __init__(self,
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(1, dim//2, kernel_size=3, stride=1, padding="same"),
            nn.LeakyReLU()
        )
        self.layernorm = LayerNorm(dim//2, eps=1e-6)
        self.down = nn.Conv2d(dim, dim//2, kernel_size=3, stride=1, padding="same")

    def forward(self, x, H, W, scale_input):
        scale = self.layer1(scale_input)
        x = torch.cat((x, scale), axis=1)
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.norm is not None:
            ...
            x = x.flatten(2).transpose(1, 2)
            x = self.norm(x)
            x = x.transpose(1, 2).view(-1, self.dim[-1], Wh, Ww)
...
class encoder(nn.Module):
    ...
    def forward(self, x, scale_input):
        """Forward function."""
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, H, W, x, Wh, Ww = layer(x, Wh, Ww, scale_input[i])
...
class Block(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, input_resolution=None, num_heads=None,
                 window_size=None, i_block=None, qkv_bias=None, qk_scale=None, channelAttention_reduce=4):
        super().__init__()
        self.bn = nn.BatchNorm2d(num_features=dim)
    ...
    def forward(self, x, mask):
        ...
        x3 = input + self.drop_path(x2)
        x3 = x3.permute(0, 2, 3, 1)  # (N, C, H, W) -> (N, H, W, C)
        x3 = self.blocks_tr(x3, mask)
        x3 = x3.permute(0, 3, 1, 2).contiguous()

        return x3


class CAUNet(SegmentationNetwork):
    def __init__(self,
                 config,
                 num_input_channels,
                 embedding_dim,
        ...
        # Multi-scale input
        self.scale1 = nn.AvgPool2d(4, 4)
        self.scale2 = nn.AvgPool2d(2, 2)

    def forward(self, x):
        # Multi-scale input
        scale_img_2 = self.scale1(x)  # x shape[batch_size,channel(1),224,224]->shape[batch,1,56,56]
        scale_img_3 = self.scale2(scale_img_2)  # shape[batch,1,28,28]
        scale_img_4 = self.scale2(scale_img_3)  # shape[batch,1,14,14]
        scale_img_5 = self.scale2(scale_img_4)
        scal_img = [scale_img_2, scale_img_3, scale_img_4, scale_img_5]
        seg_outputs = []
        skips = self.encoder(x, scal_img)
        neck = skips[-1]
