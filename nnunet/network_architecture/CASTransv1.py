...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, scale, relative_pos_enc=None):
        ...
        #Multi scale input
        x0 = torch.cat((self.layer(scale), x0), axis=1)
        # x = self.bn(x)
        # #Multi scale input
...
class encoder(nn.Module):
    def __init__(self,
                 ...
                 embed_dim=96,
                 depths=[3, 3, 3, 3],
                 channelAttention_reduce=[4, 4, 4, 4],
                 num_heads=[3, 6, 12, 24],
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                channelAttention_reduce=channelAttention_reduce[i_layer],
...
class MultiHeadAttention(nn.Module):
    def __init__(self,
        ...
        # self.attention = nn.MultiheadAttention(embed_dim=channels, num_heads=num_heads, batch_first=True, dropout=proj_drop)
        self.conv = nn.Conv2d(channels, channels, 3, 1, padding="same")
        self.attention = nn.MultiheadAttention(embed_dim=channels, 
                                               bias=attention_bias, 
                                               batch_first=True,
                                               # dropout = 0.0,
                                               num_heads=num_heads)#num_heads=self.num_heads)
...
class Block(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, input_resolution=None, num_heads=None,
        ...
        # self.ln = nn.LayerNorm([dim, input_resolution[0], input_resolution[0]], eps=1e-5)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
    ...
    def forward(self, x):
        input = x.clone()
        ch, mhsa = self.attention(x)
        mhsa = input + self.drop_path(mhsa)
        sa = self.bn(mhsa)
        sa = self.ffn_block(sa)
        o = mhsa + self.drop_path(sa)
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = spatial_att * x_init
        out = self.conv(out)
        return out


class ChannelPool(nn.Module):
    '''对通道同时进行最大和平均池化，合并成一个通道，之后进行一个拼接。目的是为了计算空间注意力'''
    def forward(self, x):
...
class BiFusion_block(nn.Module):
    ...
    def __init__(self, ch_1, ch_2, r_2, ch_int, ch_out, drop_rate=0.):
        '''ch_1:cnn输入通道
           ch_2:transformer输入通道
           r_2:通道降维倍数
           ch_int:双线性建模通道输入到residual的通道
           ch_out:residual输出通道
           drop_rate:dropout层的概率
        '''
        super(BiFusion_block, self).__init__()

        # channel attention for F_g(transformer), use SE Block
        #先降维后升维，之后sigmoid激活
    ...
    def forward(self, g, x):
        # bilinear pooling
        W_g = self.W_g(g)    #cnn，通道变化
        W_x = self.W_x(x)    #transformer，通道变化
        bp = self.W(W_g*W_x) #二者相乘

        # spatial attention for cnn branch
        g_in = g
        g = self.compress(g) #challelpool，cnn
        g = self.spatial(g)  #conv通道变化，cnn
...
class CASCADE(nn.Module):
    def __init__(self, channels=[512, 320, 128, 64], channelAttention_reduce=16, num_heads=[16, 8, 4, 2],
        ...
        self.fusion4 = BiFusion_block(ch_1=channels[0], ch_2=channels[0], r_2=4, ch_int=channels[0], ch_out=channels[0], drop_rate=0.1)
        self.fusion3 = BiFusion_block(ch_1=channels[1], ch_2=channels[1], r_2=4, ch_int=channels[1], ch_out=channels[1], drop_rate=0.1)
        self.fusion2 = BiFusion_block(ch_1=channels[2], ch_2=channels[2], r_2=2, ch_int=channels[2], ch_out=channels[2], drop_rate=0.1)
        self.fusion1 = BiFusion_block(ch_1=channels[3], ch_2=channels[3], r_2=1, ch_int=channels[3], ch_out=channels[3], drop_rate=0.1)
        # self.mix4 = MixAttentionBlock(channels[0], channels[0], channelAttention_reduce, img_size[0]//32, num_heads[0], drop_path=dpr[0])
        # self.mix3 = MixAttentionBlock(channels[1], channels[1], channelAttention_reduce, img_size[0]//16, num_heads[1], drop_path=dpr[1])
        # self.mix2 = MixAttentionBlock(channels[2], channels[2], channelAttention_reduce, img_size[0]//8, num_heads[2], drop_path=dpr[2])
        # self.mix1 = MixAttentionBlock(channels[3], channels[3], channelAttention_reduce, img_size[0]//4, num_heads[3], drop_path=dpr[3])
        # self.ffn4 = FFNBlock(channels[0], channels[0]*4, channels[0])
        # self.ffn3 = FFNBlock(channels[1], channels[1]*4, channels[1])
        # self.ffn2 = FFNBlock(channels[2], channels[2]*4, channels[2])
        # self.ffn1 = FFNBlock(channels[3], channels[3]*4, channels[3])
        ...
        # self.ln4 = LayerNorm(channels[0], data_format="channels_first", eps=1e-5)
        # self.ln3 = LayerNorm(channels[1], data_format="channels_first", eps=1e-5)
        # self.ln2 = LayerNorm(channels[2], data_format="channels_first", eps=1e-5)
        # self.ln1 = LayerNorm(channels[3], data_format="channels_first", eps=1e-5)
        # self.pfb4 = PFB(channels[0])
        # self.pfb3 = PFB(channels[1])
        # self.pfb2 = PFB(channels[2])
        # self.pfb1 = PFB(channels[3])
        self.act = nn.GELU()
        # self.act = nn.LeakyReLU()

    def forward(self, skips):
        en = skips[-1]
        # d4 = self.fusion4(g=ch[3], x=en)
        d4_c = self.bn4(en)
        d4_c = self.Conv_1x1(d4_c)
        d4_c = self.act(d4_c)
        # CAM4
        d4_c = self.CA4(d4_c) * d4_c
        # B, C, H4, W4 = d4.shape
        d4_s = self.SA4(d4_c) * d4_c
        # d4c, d4 = self.mix4(en)
        # d4 = self.ln4(d4)
        # d4 = self.ffn4(d4)
        # d4 = self.fusion4(g=d4c, x=d4)
        # d4 = self.pfb4(d4, d4c)
        # upconv3 1024*7*7->512*14*14 
        d4 = self.ConvBlock4(d4_s)
        d3 = self.Up3(d4)
        ...
        x3 = self.fusion3(g=d3, x=skips[2])

        # # Concat 3
        d3 = torch.cat((x3, d3), axis=1)
        d3_c = self.bn4(d3)
        d3_c = self.CA3(d3_c) * d3_c
        d3_s = self.SA3(d3_c) * d3_c
        # CAM3
        # d3c, d3 = self.mix3(d3)
        # d3 = self.ln3(d3)
        # d3 = self.ffn3(d3)
        # upconv2 512*14*14->256*28*28
        d3 = self.ConvBlock3(d3_s)  # 512*14*14
        d2 = self.Up2(d3)
        # AG2
        # x2 = self.AG2(g=d2, x=skips[1])
        # d3 = self.pfb3(d3, d3c)  # 1024*14*14
        x2 = self.fusion2(g=d2, x=skips[1])
        # Concat 2 512*28*28
        d2 = torch.cat((x2, d2), axis=1)
        d2_c = self.bn3(d2)
        d2_c = self.CA2(d2_c) * d2_c
        d2_s = self.SA2(d2_c) * d2_c
        ...
        d2 = self.ConvBlock2(d2_s)  # 512*28*28->256*28*28
        d1 = self.Up1(d2)
        # d2 = self.pfb2(d2, d2c)
        ...
        x1 = self.fusion1(g=d1, x=skips[0])

        # Concat 1 256*56*56
        d1 = torch.cat((x1, d1), axis=1)
        d1_c = self.bn2(d1)
        # CAM1
        d1_c = self.CA1(d1_c) * d1_c
        d1_s = self.SA1(d1_c) * d1_c
        ...
        d1 = self.ConvBlock1(d1_s)  # ->128*28*28
        return d4, d3, d2, d1
...
class CASTransv1(SegmentationNetwork):
    def __init__(self,
                 config,
                 num_input_channels,
                 embedding_dim,
                 ...
                 deep_supervision,
                 conv_op=nn.Conv2d):
        super(CASTransv1, self).__init__()
        ...
        # decoder initialization self.num_heads[::-1][0:]
        self.decoder = CASCADE(channels=self.filters, channelAttention_reduce=4,
                               num_heads=[2, 2, 2, 2], img_size=self.crop_size,
    ...
    def forward(self, x):
        ...
        x1_o, x2_o, x3_o, x4_o = self.decoder(skips)
        p1 = self.dropout(self.segmentation_head1(x1_o)) #7*7
        p2 = self.dropout(self.segmentation_head2(x2_o)) #14*14
        p3 = self.dropout(self.segmentation_head3(x3_o)) #28
        p4 = self.dropout(self.segmentation_head4(x4_o))  #56
