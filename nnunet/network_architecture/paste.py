...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, scale):
        ...
        if self.downsample is not None:
            ...
            Wh, Ww = (H + 1) // 2, (W + 1) // 2
            return x1, H, W, x_down, Wh, Ww
        else:
            return x1, H, W, x1, H, W
...
class RepBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # x = self.conv(x)
        # x = self.act(x)
        channel_att_vec = self.ca(x_c)
        channel = channel_att_vec * x_c
        x_s = inputs.permute(0, 2, 3, 1).contiguous()
        spatial = self.swtrans(x_s)
        spatial = spatial.permute(0, 3, 1, 2).contiguous()
        # out = spatial + channel
        out = self.pfb(spatial, channel)
        # out = self.conv(out)
        return out
...
class Block(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, input_resolution=None, num_heads=None,
        ...
        self.repmlp_block = RepBlock(in_channels=dim, out_channels=dim, channelAttention_reduce=channelAttention_reduce,
                                     ...
                                     shift_size=shift_size, qkv_bias=qkv_bias, qk_scale=qk_scale, drop_path=drop_path)
        self.ffn_block = FFNBlock(dim, dim * ffn_expand)

    def forward(self, x):
        input = x.clone()
        # x1 = self.bn(x)
        x1 = self.repmlp_block(x)
        x1 = input + self.drop_path(x1)
        x2 = self.bn(x1)
        x2 = self.ffn_block(x2)
        out = x1 + self.drop_path(x2)
        return out
...
class CASCADE(nn.Module):
    def __init__(self, channels=[512, 320, 128, 64], channelAttention_reduce=16, num_heads=[16, 8, 4, 2], img_size=[224, 224], window_size=[7, 7, 7, 7]):
        ...
        # self.SA1 = SwinTransformerBlock(2 * channels[3], input_resolution=(img_size[0] // 4, img_size[0] // 4),
        #                                 num_heads=num_heads[3], window_size=window_size[3], shift_size=0)
        # self.SA2 = SwinTransformerBlock(2*channels[2], input_resolution=(img_size[0] // 8, img_size[0] // 8),
        #                                 num_heads=num_heads[2], window_size=window_size[2], shift_size=0)
        # self.SA3 = SwinTransformerBlock(2*channels[1], input_resolution=(img_size[0] // 16, img_size[0] // 16),
        #                                 num_heads=num_heads[1], window_size=window_size[1], shift_size=0)
        # self.SA4 = SwinTransformerBlock(channels[0], input_resolution=(img_size[0] // 32, img_size[0] // 32),
        #                                 num_heads=num_heads[0], window_size=window_size[0], shift_size=0)
        ...
        self.ln3 = LayerNorm(channels[1], data_format="channels_first")
        self.ln2 = LayerNorm(channels[2], data_format="channels_first")
        self.ln1 = LayerNorm(channels[3], data_format="channels_first")
    ...
    def forward(self, x, skips):
        ...
        # B, C, H4, W4 = d4.shape
        d4_s = self.ln4(x)
        d4_s = self.Conv_1x1(d4_s)
        d4_s = self.act(d4_s)
        # d4_s = d4_s.permute(0, 2, 3, 1)
        # d4_s = self.SA4(d4_s)
        # d4_s = d4_s.permute(0, 3, 1, 2)
        d4_s = self.SA4(d4_s) * d4_s
        d4 = self.pfb4(d4_s, d4_c)
        ...
        d3_s = self.SA3(d3_s) * d3_s
        d3 = self.pfb3(d3_s, d3_c)
        ...
        d2_s = self.SA2(d2_s) * d2_s
        d2 = self.pfb2(d2_s, d2_c)
        ...
        d1_s = self.SA1(d1_s) * d1_s
        d1 = self.pfb1(d1_s, d1_c)
...
class CASTransv1(SegmentationNetwork):
    def __init__(self,
        ...
        self.decoder = CASCADE(channels=self.filters, channelAttention_reduce=16,
                               num_heads=self.num_heads[::-1][0:], img_size=self.crop_size,
